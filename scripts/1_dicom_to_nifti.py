...
def _fix_series_paths(series_paths: dict) -> dict:
    ...
    _IGNORE_DIRS = {
        ...
        "node_modules", "models", "results", "scripts", "config",
        ".agents", ".gemini", "nifti", "monai_ready", "extra_sequences",
        "brats_bundle", "site-packages", "lib", "bin", "include",
    }
    ...
    print(f"  Searching for DICOM folders in project...")
    ...
    if remapped > 0:
        ...
        if current_sess_path.exists():
            ...
            print(f"  ⚠️  Could not find current session file to update")
...
def check_dcm2niix() -> str:
    ...
    for c in ["/opt/homebrew/bin/dcm2niix",
...
def convert_series(dcm2niix: str, dicom_dir: Path,
                   out_dir: Path, out_name: str) -> bool:
...
if not valid_paths:
    print(f"❌  No DICOM series folders found.")
    print(f"")
    print(f"  The session has {len(SERIES_PATHS)} series but none exist at stored paths.")
    print(f"  This happens when the project was moved after the wizard ran.")
    print(f"")
    print(f"  Your DICOM folder should be at:")
    ...
    if dicom_root:
        ...
        ans = input(f"  Use this folder? [Y/n]: ").strip().lower()
        if ans in ("", "y", "yes"):
        ...
        print(f"  Could not auto-detect DICOM folder.")
        print(f"  Please re-run the wizard:  python3 run_pipeline.py")
        sys.exit(1)
...
for role, folder in ASSIGNMENTS.items():
    ...
    all_files = [f for f in dicom_dir_path.rglob("*")
                 if f.is_file() and not f.name.startswith(".")]
    if not all_files:
...
try:
    ...
    if not t1_path.exists():
        ...
        for src, dst in [("t1c.nii.gz",   "t1c_resampled.nii.gz"),
            ...
            print(f"✅  metric={metric:.4f}" if metric == metric
                  else "✅  (geometry resample)")

        # Extra sequences — resample only (no registration)
        for src, dst in [("t2star.nii.gz", "t2star_resampled.nii.gz"),
...
try:
    ...
    for name, path in required.items():
        if path.exists():
            ...
            print(f"  ✅ {name:5s}: shape={img.shape}  "
                  f"voxel={tuple(round(float(v), 2) for v in vox)} mm") # type: ignore
        else:
    ...
    for fname in ["t2star_resampled.nii.gz",
