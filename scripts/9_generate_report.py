...
def metrics_table(metrics: dict):
    ...
    def grade_str(metric, value):
        ...
        if c == colors.HexColor("#2e7d32"): return "GOOD"
        if c == colors.HexColor("#e65100"): return "MODERATE"
        return "POOR"

    headers = ["Metric", "Value", "Threshold", "Grade"]
    rows    = [
        ["Dice score",         f"{metrics.get('dice',0):.4f}",
...
def make_header_footer(canvas, doc):
    ...
    canvas.drawString(MARGIN, PAGE_H - 1.0*cm,
                      "BRAIN TUMOUR ANALYSIS — AI RESEARCH REPORT")
...
def build_report():
    ...
    story.append(Paragraph("Brain Tumour Analysis Report by Soares", S["title"]))
    story.append(Paragraph(
        "AI-Assisted Segmentation — Research Use Only", S["subtitle"]))
    story.append(Spacer(1, 0.3*cm))
    story.append(HR)
    story.append(Spacer(1, 0.3*cm))
    ...
    story.append(Paragraph(
        "Direct baseline reference from clinical records used for AI alignment:",
        S["body"]
    ))
    ...
    findings_rows = [
        ...
        ["Ref. Volume",     f"{PATIENT.get('radiologist_volume_cc', 32.0):.1f} cc"],
        ["Target Area",     "Left fronto-parietal/rolandic & Right frontal"],
    ...
    v_table = volume_table(
        volume_rows,
        ["Region", "AI Volume", "Reference", "Difference"]
    )
    if u_color:
    ...
    if diff_pct >= 20:
        ...
        discrepancy_para = (
            "<b>Understanding the volume discrepancy:</b> The radiologist's estimate of 32 cc "
            "is a <i>clinical approximation</i> based on manual ellipsoid measurement "
            "(major axes: 4.3 × 3.0 × 2.5 cm → geometric volume = "
            "<sup>4</sup>/<sub>3</sub> × π × 2.15 × 1.5 × 1.25 = <b>16.9 cc</b>). "
            "The AI whole-tumour volume of {v_w:.1f} cc includes all three BraTS labels "
            "(necrotic core + edema + enhancing tumour), producing a larger total because "
            "pathological edema extends beyond the solid tumour boundary. "
            "The AI <i>core</i> volume (necrotic + enhancing = {v_core:.1f} cc) closely "
            "matches the geometric ellipsoid (16.9 cc), confirming geometric accuracy. "
            "The remaining gap ({v_edema:.1f} cc edema) reflects the inherent difference "
            "between a solid-tumour geometric estimate and a whole-tumour volumetric "
    ...
    if location and "error" not in location:
        ...
        loc_rows = [
            ...
            ["Max diameter",      f"{float(location.get('max_diameter_mm',0)):.0f} mm  "
                                  f"({float(location.get('max_diameter_mm',0))/10:.1f} cm)"],
            ["Bounding box",      f"X={location['bounding_box_mm'].get('x_mm',0):.0f}mm  "
            ...
            ["Centre (RAS mm)",   f"R={location['centre_ras_mm'][0]}  " # type: ignore
            ...
            ["Midline crossing",  "YES — bilateral involvement"
                                  if location.get('crosses_midline') else "No"],
    ...
    add_viz_image(
        poverlay,
        caption="🔵 Necrotic core   🟢 Peritumoural Edema   🔴 Enhancing tumor"
    )
    ...
    if p3d_nav.exists():
        ...
        if location and "centre_ras_mm" in location:
            ...
            story.append(info_table([
                ["Tumour Centroid RAS",
                 f"R = {ras[0]:.0f} mm   A = {ras[1]:.0f} mm   S = {ras[2]:.0f} mm"],
                ["Navigation Reference",
                 "Stereo tactic frame: L = -R, P = -A, S = -S (RAS convention)"],
            ]))
            story.append(Spacer(1, 0.2*cm))
    ...
    if p3d.exists():
        link_rows.append(["3D Surface View", "viz_3d_surface.html", "Interactive Plotly mesh (necrotic, edema, enhancing)"])
    if panim.exists():
        link_rows.append(["Slice Animation", "viz_slice_animation.html", "Frame-by-frame scrollable viewer (200MB file)"])
    ...
    if morphology and "shape" in morphology:
        ...
        if tf:
            ...
            mm = "POSITIVE (FLAIR > T2 hyperintensity consistent with tumour)" if tf.get("mismatch_sign") else "NEGATIVE"
            tf_rows = [
