...
PATIENT_TEMPLATE = {
    ...
    "age": "Desconhecido",
    "ref": "REF-000000",
    "exam_date": "Desconhecido",
    "institution": "Nome da Instituição",
    "radiologist": "Médico Referenciador",
...
def metrics_table(metrics: dict):
    ...
    rows = [
        ["Score Dice", f"{metrics.get('dice', 0):.4f}", "> 0.80", grade_str("dice", metrics.get("dice", 0))],
        [
            "HD95 (mm)",
        ...
        ["Especificidade", f"{metrics.get('specificity', 0):.4f}", "> 0.99", ""],
...
def make_header_footer(canvas, doc):
    ...
    canvas.drawString(MARGIN, PAGE_H - 1.0 * cm, "ANÁLISE DE TUMOR CEREBRAL — RELATÓRIO DE INVESTIGAÇÃO IA")
    canvas.setFont("Helvetica", 8)
    canvas.drawRightString(
        PAGE_W - MARGIN, PAGE_H - 1.0 * cm, f"Página {doc.page}  |  {datetime.now().strftime('%d/%m/%Y')}"
    )
...
def build_report():
    ...
    story.append(Paragraph("Segmentação Assistida por IA — Apenas para Investigação", S["subtitle"]))
    story.append(Spacer(1, 0.3 * cm))
    story.append(HR)
    story.append(Spacer(1, 0.3 * cm))
    ...
    story.append(Paragraph("Referência clínica direta utilizada para alinhamento da IA:", S["body"]))
    story.append(Spacer(1, 0.2 * cm))

    # Session-aware findings display
    ref_vol_display = f"{PATIENT.get('radiologist_volume_cc'):.1f} cc" if PATIENT.get("radiologist_volume_cc") else "—"
    findings_rows = [
        ["Base Clínica", Paragraph(findings_text, S["body_small"])],
        ["Volume Ref.", ref_vol_display],
        ["Área Alvo", location.get("lobes", "Não computado")],
        ["Mass Effect", "Ver análise IA abaixo"],
    ]
    ...
    ref_str = f"~{ref_vol:.0f} cc" if ref_vol else "—"
    diff_str = f"{abs(v_w - ref_vol) / ref_vol * 100:.0f}% diff" if ref_vol else "Sem referência"

    volume_rows = [
        ["Tumor Completo", f"{v_w:.1f} cc", ref_str, diff_str],
    ...
    v_table = volume_table(volume_rows, ["Região", "Volume IA", "Referência", "Diferença"])
    if u_color:
        v_table.setStyle(
            TableStyle(
                [
    ...
    if ref_vol:
        ...
        if diff_pct < 5:
            match_text = (
                f"<b>Excelente correspondência de volume:</b> IA mediu {v_w:.1f} cc "
                f"vs radiologista {ref_str} ({diff_pct:.1f}% diferença). "
                f"Volume de segmentação clinicamente preciso."
            )
            match_color = "#2e7d32"
        elif diff_pct < 20:
            match_text = (
                f"<b>Boa correspondência de volume:</b> IA mediu {v_w:.1f} cc "
                f"vs radiologista {ref_str} ({diff_pct:.1f}% diferença)."
            )
            match_color = "#e65100"
        else:
            match_text = (
                f"<b>Discrepância de volume:</b> IA mediu {v_w:.1f} cc "
                f"vs radiologista {ref_str} ({diff_pct:.1f}% diferença). "
                f"Correção manual recomendada."
            )
            match_color = "#c62828"
    else:
        match_text = f"<b>Volume do tumor completo medido pela IA:</b> {v_w:.1f} cc. Sem volume de referência do radiologista disponível."
        match_color = "#555555"
    ...
    if location and "error" not in location:
        ...
        loc_rows = [
            ...
            ["Cruzamento da Linha Média", "SIM — envolvimento bilateral" if location.get("crosses_midline") else "Não"],
    ...
    add_viz_image(poverlay, caption="�� Núcleo Necrótico   🟢 Edema Peritumoral   🔴 Tumor com Realce")
    ...
    if p3d.exists():
        link_rows.append(
            [
                ...
                "viz_3d_surface.html",
                "Interactive Plotly mesh (necrotic, edema, enhancing)",
            ]
        )
    if panim.exists():
        link_rows.append(
            ["Animação por Cortes", "viz_slice_animation.html", "Frame-by-frame scrollable viewer (200MB file)"]
        )
    ...
    if radiomics and "classification" in radiomics:
        ...
        if ml_rows:
            ...
            if "primary_vs_metastasis" in c and "interpretation" in c["primary_vs_metastasis"]:
                story.append(
                    Paragraph(f"<b>Primary vs Met:</b> {c['primary_vs_metastasis']['interpretation']}", S["body"])
                )
    ...
    # ── Molecular Status Prediction (Imaging-Based) ──────────────────
    mol_path = latest_dir / "molecular_prediction.json"
    if mol_path.exists():
        try:
            with open(mol_path) as f:
                mol = json.load(f)

            story.append(Paragraph("Previsão Molecular por Imagem (IDH / MGMT)", S["section"]))
            story.append(
                Paragraph(
                    "Previsão não invasiva do estado mutacional IDH e metilação MGMT baseada "
                    "em padrões de imagem correlacionados com biologia tumoral. "
                    "<b>Método: Heurístico radiômico v1 — apenas para investigação.</b>",
                    S["body"],
                )
            )
            story.append(Spacer(1, 0.2 * cm))
            ...
            mol_rows = [
                [
                    "Estado IDH",
                    f"<b>{idh.get('prediction', 'unknown').upper()}</b>",
                    f"{idh.get('probability', 0):.0%}",
                    idh.get("confidence_level", "—"),
                ],
                [
                    "Metilação MGMT",
                    f"<b>{mgmt.get('prediction', 'unknown').upper()}</b>",
                    f"{mgmt.get('probability', 0):.0%}",
                    mgmt.get("confidence_level", "—"),
                ],
            ]

            mol_table = Table(
                [["Biomarcador", "Previsão", "Prob.", "Conf."]] + mol_rows, colWidths=[4 * cm, 4 * cm, 3 * cm, 5 * cm]
            )
            mol_table.setStyle(
                TableStyle(
                    [
    ...
    model_rows = [
        ...
        [
            "2. Classification",
            "XGBoost Classifier",
            "Trained on BraTS radiomics features for grade/subtype prediction.",
        ],
        [
            "3. Genomics",
            "BrainIAC Foundation",
