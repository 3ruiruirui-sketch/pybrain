...
def generate_synthetic_cohort(
    ...
    for i, name in enumerate(feature_names):
        if "ratio" in name or "sphericity" in name or "convexity" in name \
           or "elongation" in name or "flatness" in name or "extent" in name:
...
def train_model(X: np.ndarray, y: np.ndarray, feature_names: list) -> dict:
    ...
    try:
        ...
        if torch.backends.mps.is_available():
            ...
            tree_method = "hist"
            device_str = "cpu"  # XGBoost MPS support is experimental
    ...
    n_cnn_important = sum(
        1 for idx in top_idx if feature_names[idx].startswith("cnn_feat_")
    )
...
def save_model(model, feature_names: list, metrics: dict):
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"  AUC-ROC:  {metrics['cv_auc_mean']:.4f} "
...
if __name__ == "__main__":
    ...
    has_real_data = (
        (real_data_path / "radiomics_features.npy").exists()
        and (real_data_path / "labels.npy").exists()
    )
    ...
    if has_real_data:
        ...
        X, y, feature_names = generate_synthetic_cohort(
            n_hgg=250, n_lgg=150, seed=42
        )
    ...
    print(f"  Run Stage 8 to use it: python3 scripts/8_radiomics_analysis.py")
    print()
