...
def validate_uncertainty_in_pipeline():
    ...
    model_probs_list = [
        np.random.rand(3, 96, 96, 96) * 0.8 + 0.1 for _ in range(3)
    ]
    
    # Test current pipeline logic
    if model_probs_list:
        uncertainty = compute_uncertainty(ensemble_prob, model_probs_list)
    ...
    try:
        # Single model case
        single_uncertainty = compute_uncertainty(ensemble_prob, [ensemble_prob])
        logger.info("✅ Single model uncertainty computed")
        
        # Empty model list (should not crash)
        empty_uncertainty = compute_uncertainty(ensemble_prob, [])
...
def validate_uncertainty_clinical_relevance():
    ...
    if confidence_trend:
        ...
        if low_conf_uncertainty > high_conf_uncertainty:
            ...
            logger.info(f"High: {high_conf_uncertainty:.3f}, Edge: {edge_uncertainty:.3f}, Low: {low_conf_uncertainty:.3f}")
            return False
