...
if qual_path.exists():
    with open(qual_path) as f:
        ...
        if non_gbm:
            ...
            with open(OUTPUT_DIR / "morphology_results.json", "w") as f:
                json.dump(
...
if seg_path is None:
    raise ValueError("Segmentation path is None")
...
try:
    ...
    print(
        f"  NIfTI orientation: {orient_codes}  (axis0={orient_codes[0]}, axis1={orient_codes[1]}, axis2={orient_codes[2]})"
    )
except Exception:
    orient_codes = ("?", "?", "?")
...
if ncomp > 0:
    ...
    for p in props_list:
        ...
        if vol_cc >= 0.1:
            print(
                f"    Comp {p.label}: {vol_cc:.2f}cc, centroid=({cx:.0f},{cy:.0f},{cz:.0f}), diam={diam:.0f}mm{marker}"
            )
...
n_et = int(np.sum(seg_arr == 4))  # BraTS 2021: ET = label 4
n_tc = n_ncr + n_et
...
if t1 is not None and t1c is not None:
    ...
    core_b = np.asarray(seg_arr == 4).astype(bool)  # enhancing only (BraTS 2021: ET = 4)
    if core_b.any():
...
if t2 is not None and flair is not None:
    ...
    if mismatch_sign:
        ...
        print("                       → IDH-mutant signature present")
    else:
        print(f"  T2-FLAIR mismatch  : No (score={mismatch_score:.3f})")
        print("                       → IDH-wildtype more likely")
...
if sphericity < 0.55:
    interp.append(
        (
            "⚠️ ",
            "LOW sphericity — irregular infiltrative shape typical of "
    ...
    interp.append(
        (
            "ℹ️ ",
            "MODERATE-HIGH sphericity — relatively round lesion. "
...
if convexity < 0.75:
    interp.append(
        (
            "⚠️ ",
            "LOW convexity — tumour has concave/infiltrative borders. "
...
if ncr_frac > 0.15:
    interp.append(
        (
            "⚠️ ",
            f"HIGH necrosis fraction ({ncr_frac * 100:.1f}%) — "
    ...
    interp.append(
        ("ℹ️ ", f"Moderate necrosis ({ncr_frac * 100:.1f}%) — present but limited. Can be seen in Grade III-IV.")
    )
...
if et_frac > 0.15:
    interp.append(
        (
            "⚠️ ",
            f"HIGH enhancement fraction ({et_frac * 100:.1f}%) — "
...
if ed_core > 4.0:
    interp.append(
        (
            "ℹ️ ",
            f"HIGH oedema-to-core ratio ({ed_core:.1f}) — "
    ...
    interp.append(
        ("ℹ️ ", f"LOW oedema-to-core ratio ({ed_core:.1f}) — limited oedema. Slightly more typical of metastasis.")
    )
...
if roughness > 3.0:
    interp.append(
        (
            "ℹ️ ",
            f"HIGH surface roughness ({roughness:.1f}mm) — "
...
if t1c is not None:
    ...
    patient_name = _sess.get("patient", {}).get("name", "Patient")
    fig.suptitle(f"Tumour Morphology Analysis — {patient_name}", color="white", fontsize=12, y=0.98)
    _gs_fig: Any = gs
    ...
    def show_slice(ax, vol, mask, title, cmap="gray", overlay_cmap=None):
        sl = vol[:, :, best_z].T
        msl = mask[:, :, best_z].T
        ax.imshow(sl, cmap=cmap, vmin=np.percentile(sl, 2), vmax=np.percentile(sl, 98))
    ...
    metrics_names = ["Sphericity", "Convexity", "Elongation", "NCR frac", "ET frac", "ED/core"]
    metrics_vals = [sphericity, convexity, elongation, min(ncr_frac * 5, 1), min(et_frac * 5, 1), min(ed_core / 10, 1)]
    colors_bar = ["#4499ff", "#44ee44", "#ff9900", "#ff4444", "#ff8888", "#aaaaff"]
    ...
    if adc_path.exists():
        ...
        if len(adc_vals) > 10:
            ...
            ax_h.axvline(800, color="#ff4444", lw=1.5, linestyle="--", label="High grade (<800)")
            ax_h.axvline(1200, color="#ffaa00", lw=1.5, linestyle="--", label="Moderate (800-1200)")
            adc_median = float(np.median(adc_vals))
            ax_h.axvline(adc_median, color="white", lw=1, linestyle=":", label=f"Median={adc_median:.0f}")
            ax_h.set_title("ADC histogram (tumour+periphery)", color="white", fontsize=8, pad=3)
            ax_h.tick_params(colors="white", labelsize=7)
            ax_h.spines[:].set_color("#444")
            ax_h.set_xlabel("ADC value", color="#aaa", fontsize=7)
            ax_h.legend(fontsize=6, labelcolor="white", facecolor="#222", edgecolor="#444")
        else:
            ax_h.text(
                0.5, 0.5, "ADC: too few voxels", color="white", ha="center", va="center", transform=ax_h.transAxes
            )
            ax_h.axis("off")
    else:
        ax_h.text(0.5, 0.5, "ADC not available", color="white", ha="center", va="center", transform=ax_h.transAxes)
        ax_h.axis("off")
...
banner("DONE")
print("  Open morphology_analysis.png to see the visualisation.")
print("  Run generate_report.py to include in PDF report.")
