...
class Preflight:
    ...
    def check_layer2(self):
        ...
        for label, path in required.items():
            if not path.exists():
                if is_stage_1:
                    self.log("INPUT", f"{label} NIfTI", "WARN", f"Missing, but Stage 1 will generate it.")
                else:
    ...
    def check_layer5(self):
        ...
        if "T1" in self.vols and "T1c" in self.vols and "SEG" in self.vols:
            seg_data = self.vols["SEG"].get_fdata().astype(np.uint8)
            enh_mask = (seg_data == 4)  # BraTS 2021 ET label is 4, not 3
        ...
        if "ADC" in self.vols and "SEG" in self.vols:
            ...
            if seg_img.shape == adc_img.shape:
                ...
                self.log("CLINICAL", "ADC Tumour", "WARN", f"Shape mismatch ({adc_img.shape} vs SEG {seg_img.shape}) — skipping check")
