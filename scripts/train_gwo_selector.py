...
def load_data():
    ...
    if DATA_PATH.exists():
        ...
        X, y = make_classification(
            n_samples=400, n_features=N_FEATURES_TOTAL, n_informative=15, n_redundant=20, random_state=42
        )
...
class BinaryGreyWolfOptimizer:
    ...
    def hunt(self):
        banner("BGWO: HUNTING THE GOLDEN FEATURE SUBSET (183)")
        ...
        for t in range(self.max_iter):
            ...
            print(
                f"  Iteration {t + 1:02d}/{self.max_iter} — pack consensus: {100 * alpha_accuracy:.1f}% Acc | {int(self.alpha_pos.sum())}/{self.n_features} Feats"
            )
