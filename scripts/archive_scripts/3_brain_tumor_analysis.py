...
try:
    ...
    # Load checkpoint
    ckpt_files = list((bundle_path / "models").glob("*.pt")) + list((bundle_path / "models").glob("*.pth"))
    if not ckpt_files:
...
if seg_source == "bundle" and "_raw_probs" in dir():
    ...
    if _rel_diff > MATCH_TOLERANCE:
        ...
        print(f"      Best candidate: WT={best_thresh} → {_best_vol:.1f} cc ({_rel_diff * 100:.0f}% off).")
        print(f"      → AUTO-FALLBACK to permissive threshold {FALLBACK_THRESH} to capture infiltrative tumour.")
    ...
    print("  Updated volumes with best threshold:")
    print(f"     Whole tumor     : {v_whole:.1f} cc")
    print(f"     Necrotic core   : {v_necrotic:.1f} cc")
    print(f"     Edema           : {v_edema:.1f} cc")
...
for name, vol in vols.items():
    ...
    stats[name] = {
        k: float(v) for k, v in zip(["mean", "std", "min", "max"], [vals.mean(), vals.std(), vals.min(), vals.max()])
    }
...
if extra_vols and tumor_bool.any():  # type: ignore
    ...
    for ename, evol in extra_vols.items():
        ...
        extra_stats[ename] = {
            kk: float(vv)
            for kk, vv in zip(  # type: ignore
                ["mean", "std", "min", "max"], [evals.mean(), evals.std(), evals.min(), evals.max()]
            )
        }  # type: ignore
        hint = ""
        if ename == "ADC":
            ...
            m = extra_stats[ename]["mean"]
            hint = f"Mean {m:.0f} HU — " + (
                "hyperdense tumour" if m > 40 else "isodense" if m > 20 else "hypodense/cystic"
            )
        elif ename == "CT_Calc":
        ...
        print(
            f"     {ename:6s}: mean={extra_stats[ename]['mean']:.1f}  "  # type: ignore
...
def save_grid(
    ...
    fig.suptitle(f"Brain Tumor — {axis_name}  ({seg_source})", color="white", fontsize=12, y=1.01)
    ...
    for r, row_label in enumerate(row_labels):
        for c, sl in enumerate(slices):
            ...
            if c == 0:
                ax.set_ylabel(row_label, color="white", fontsize=7, rotation=0, labelpad=42, va="center")
...
if HAS_PLOTLY:
            ...
            go.Frame(
                data=[
                    go.Heatmap(
                        z=bg[::-1],
                        colorscale="gray",
                        showscale=False,  # type: ignore
                        zmin=0,
                        zmax=1,
                        name="T1c",
                    ),
                    go.Heatmap(
                        z=np.where(n_sl[::-1] > 0, 1.0, np.nan),  # type: ignore
                    ...
                    go.Heatmap(
                        z=np.where(e_sl[::-1] > 0, 1.0, np.nan),  # type: ignore
                    ...
                    go.Heatmap(
                        z=np.where(en_sl[::-1] > 0, 1.0, np.nan),  # type: ignore
    ...
    fig = go.Figure(
        data=[
            ...
            go.Heatmap(
                z=np.where(n0[::-1] > 0, 1.0, np.nan),  # type: ignore
                colorscale=[[0, "#4499ff"], [1, "#4499ff"]],
                showscale=False,
                zmin=0,
                zmax=1,
            ...
            go.Heatmap(
                z=np.where(e0[::-1] > 0, 1.0, np.nan),  # type: ignore
                colorscale=[[0, "#44ee44"], [1, "#44ee44"]],
                showscale=False,
                zmin=0,
                zmax=1,
            ...
            go.Heatmap(
                z=np.where(en0[::-1] > 0, 1.0, np.nan),  # type: ignore
                colorscale=[[0, "#ff4444"], [1, "#ff4444"]],
                showscale=False,
                zmin=0,
                zmax=1,
    ...
    fig.update_layout(  # type: ignore
        title=dict(  # type: ignore
            text=(
                f"Brain Tumor — Interactive Viewer<br>"
                f"<span style='font-size:11px;color:#aaa'>"
                f"🔵 Necrotic &nbsp; 🟢 Edema &nbsp; 🔴 Enhancing  "
                f"({seg_source})</span>"
            ),
            x=0.5,
            font=dict(color="white", size=14),  # type: ignore
        ),
        paper_bgcolor="#111",
        plot_bgcolor="#111",
        sliders=[
            dict(  # type: ignore
                active=sl0,
                steps=steps,
                pad={"t": 50},  # type: ignore
                currentvalue={"prefix": "Axial slice: ", "font": {"color": "white", "size": 13}},  # type: ignore
        ...
        updatemenus=[
            dict(  # type: ignore
                ...
                buttons=[
                    dict(
                        label="▶ Play",  # type: ignore
                        method="animate",
                        args=[None, {"frame": {"duration": 55}, "fromcurrent": True}],
                    ),  # type: ignore
                    dict(
                        label="⏸ Pause",  # type: ignore
                        method="animate",
    ...
    try:
        ...
        try:
            ...
            if len(pred_pts) > 0 and len(gt_pts) > 0:
                ...
                hd95_v = float(np.percentile(np.concatenate([d_pg, d_gp]), 95))
            else:
                hd95_v = float("inf")
        except Exception:
            hd95_v = float("nan")
        ...
        print(f"  HD95 (mm)         : {hd95_v:.1f}     {_g(-hd95_v, -5, -15)}")
        ...
        if stats_path.exists():
            ...
            print("  Validation metrics added to tumor_stats.json")
