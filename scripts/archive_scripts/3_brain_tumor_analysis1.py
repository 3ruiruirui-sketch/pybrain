...
from pybrain.models.swinunetr import load_swinunetr, run_swinunetr_inference # type: ignore
...
def main():
    ...
    p_segresnet, p_tta4, p_swin = None, None, None
    ...
    ct_path = next((p for p in [OUTPUT_DIR / "ct_brain_registered.nii.gz", MONAI_DIR / "ct_brain_registered.nii.gz"] if p.exists()), None)
    ct_data = None
    if ct_path:
    ...
    logger.info(f"WT final segmentation volume: {compute_volume_cc((seg_full > 0).astype(np.float32), vox_vol_cc):.2f} cc")
    ...
    if n_components > 1:
        ...
        if n_removed > 0:
            ...
            logger.info(f"   Kept {n_components - n_removed} component(s), "
    ...
    stats = {
        ...
        "tumor_pct_brain": float(0.0 if compute_volume_cc(brain_mask, vox_vol_cc) <= 0 else (quality_report["v_wt_cc"] / compute_volume_cc(brain_mask, vox_vol_cc)) * 100),
