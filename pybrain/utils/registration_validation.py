...
def validate_ct_mri_registration(
    mri_data: np.ndarray, ct_data: np.ndarray, brain_mask: Optional[np.ndarray] = None, nmi_threshold: float = 1.3
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"valid": valid, "nmi": nmi, "threshold": nmi_threshold, "error": None}

    except Exception as e:
        logger.error(f"CT-MRI registration validation failed: {e}")
        return {"valid": False, "error": str(e), "nmi": 0.0, "threshold": nmi_threshold}
...
def should_apply_ct_boost(
    ...
    validation_results = validate_ct_mri_registration(mri_data, ct_data, brain_mask, nmi_threshold)
