...
class StatisticalThresholdOptimizer:
    ...
    def optimize_thresholds_uncertainty_weighted(
        ...
        for i, region in enumerate(region_names):
            ...
            uncertainty_percentiles = np.percentile(
                uncertainty[prob_map > 0.1], [25, 50, 75, 90]
            )
            ...
            if len(confident_probs) > 100:  # Minimum voxels for reliable optimization
                ...
                uncertainty_adj = self.uncertainty_weight * (
                    np.mean(uncertainty[confident_mask]) / 100.0
                )
                ...
                final_thresh = np.clip(
                    adjusted_thresh,
                    self.clinical_bounds[region][0],
                    self.clinical_bounds[region][1]
                )
                
                optimized_thresholds[region] = float(final_thresh)
                
                logger.info(
                    f"{region.upper()}: base={optimal_thresh:.3f} -> "
                    f"adjusted={final_thresh:.3f} (uncertainty_weighted)"
                )
            else:
                # Fallback to conservative threshold
                optimized_thresholds[region] = self.clinical_bounds[region][0]
                logger.warning(
                    f"{region.upper()}: Insufficient confident voxels, "
                    f"using conservative threshold"
                )
    ...
    def cross_validate_thresholds(
        ...
        for fold in range(n_folds):
            ...
            val_unc = uncertainty_maps[start_idx:end_idx]
    ...
    def validate_clinical_safety(
        ...
        for region, thresh in thresholds.items():
            ...
            in_bounds = (
                self.clinical_bounds[region][0] <= thresh <=
                self.clinical_bounds[region][1]
            )
            ...
            volume_reasonable = (
                volume_ranges[region][0] <= volume_cc <= volume_ranges[region][1]
            )
...
def adaptive_threshold_from_uncertainty(
    ...
    for region, base_thresh in base_thresholds.items():
        ...
        final_thresh = np.clip(
            adjusted,
            clinical_bounds[region][0],
            clinical_bounds[region][1]
        )
        
        adapted_thresholds[region] = float(final_thresh)
        
        logger.debug(
            f"{region.upper()}: {base_thresh:.3f} -> {final_thresh:.3f} "
            f"(uncertainty_factor={uncertainty_factor:.3f})"
        )
