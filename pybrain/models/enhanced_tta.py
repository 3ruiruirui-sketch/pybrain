...
class EnhancedTTA:
        ...
        if self.enable_rotations:
            for angle in self.rotation_angles:
                transforms.append({
                    ...
                    'inverse':   lambda x, a=angle: self._rotate_3d(
                        x.unsqueeze(0) if x.ndim == 4 else x, -a
                    ).squeeze(0) if (x.ndim == 4) else self._rotate_3d(x, -a),
                    'weight': 0.8
                })
    ...
    def _rotate_3d(self, x: torch.Tensor, angle: int) -> torch.Tensor:
        ...
        if H != W:
            raise ValueError(
                f"_rotate_3d requires square H×W slices, got H={H}, W={W}. "
                "Disable rotations for non-square inputs."
            )
    ...
    def apply_tta(self, 
                  ...
                  sw_config: Optional[Dict] = None) -> Tuple[Optional[torch.Tensor], List[str]]:
        """
...
def run_enhanced_tta_ensemble(
    ...
    # Apply TTA
    ensemble_output, applied_transforms = tta.apply_tta(
        model, input_tensor, device, inference_fn, sw_config
    )
