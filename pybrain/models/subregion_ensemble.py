...
def get_default_subregion_weights(valid_models: List[Tuple[str, np.ndarray, float]]) -> Dict[str, Dict[str, float]]:
    ...
    for region, prefs in KNOWN_PREFS.items():
        filtered_weights[region] = {name: prefs.get(name, uniform) for name in model_names}
...
def weights_from_performance(
    validation_performance: Dict[str, Dict[str, float]], model_names: List[str]
) -> Dict[str, Dict[str, float]]:
    ...
    for region, model_performances in validation_performance.items():
        ...
        if total_perf > 0:
            weights[region] = {name: model_performances.get(name, 0.0) / total_perf for name in model_names}
        else:
...
def weights_from_uncertainty(
    model_probs: List[Tuple[str, np.ndarray, float]], uncertainty_map: np.ndarray
) -> Dict[str, Dict[str, float]]:
    ...
    [name for name, _, _ in model_probs]
...
def validate_subregion_weights(subregion_weights: Dict[str, Dict[str, float]], model_names: List[str]) -> bool:
