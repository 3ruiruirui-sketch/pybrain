...
def run_mc_dropout_inference(
    ...
    mutual_info = entropy - np.mean(
        -np.sum(predictions * np.log(predictions + 1e-8), axis=1), axis=0
    )
    
    logger.info(f"MC-Dropout completed: mean std={std_prob.mean():.4f}, "
    ...
    for model_name, model in models.items():
        ...
        try:
            mean_prob, std_prob = run_mc_dropout_inference(
                model, input_tensor, device, n_samples, sw_config
            )
            ...
            blended_prob = (blend_factor * mean_prob + 
                           (1 - blend_factor) * original_prob)
...
def compute_uncertainty_metrics(
    ...
    metrics["mean_entropy"] = float(
        -np.sum(mean_prob * np.log(mean_prob + 1e-8), axis=0).mean()
    )
